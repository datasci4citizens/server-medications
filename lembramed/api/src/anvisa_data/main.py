...
def read_checkpoint(target_file):
    try:
        with open(os.path.join(f"{target_file}.json"), "r") as f:
            l = json.load(f)
            return len(l)
...
file_index = 1
while(read_checkpoint("checkpoint") != 11594):
    anvisa_dados = get_dict()
    process_nums = [
        str(dados[6]) for dados in anvisa_dados.get("data",[]) if dados[6]
    ]
    run_scraper(process_nums=process_nums,file_num=file_index)
    file_index += 1